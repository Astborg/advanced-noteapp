...
type NoteListProps = {
    availableTags: Tag[]
    notes: SimpliefiedNote[]
    deleteTag: (id: string) => void
    updateTag: (id: string, label: string) => void
}

type SimpliefiedNote = {
    tags: Tag[]
    title: string
    id: string 
}
    ...
    <>
      ...
      <Row xs={1} sm={2} lg={3} xl={4} className="g-3">
        {filteredNotes.map(note=>(
            <Col key={note.id}>
                <NoteCard id={note.id} title={note.title} tags={note.tags}></NoteCard>
            </Col>
        ))}

      </Row>
      <EditTagsModal updateTag={updateTag} deleteTag={deleteTag} show={editTagsModalIsOpen} handleClose={() => setEditTagsModalIsOpen(false)} availableTags={availableTags}></EditTagsModal>
    </>
  );
}

function NoteCard({id, title, tags}: SimpliefiedNote){
